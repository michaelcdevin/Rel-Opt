...
%     for k = 1:nIts
%         [~,NAnchs(k,j)] = Visualization_original([], 1, sqrt(nTurbs(j)),...
...
for j = 1:length(nTurbs)
    parfor k = 1:nIts
        [~,NAnchs(k,j)] = Visualization([], 1, nTurbs(j), 1451,...
            'Real multi', 5000, 0);
    end
end
